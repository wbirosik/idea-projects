...
object CassConsumer {
  ...
  def main(args: Array[String]): Unit = {
   ...
   // val sc = new SparkContext("127.0.0.1:9042", "test", conf)
    ...
    //session.execute("TRUNCATE Tweet.Twitter")
   ...
   // val _line = tmp.flatMap(_.split(","))
       val NFL = line.map(_.value) // split the message into lines
          ...
          .map(w => (w, 1L)).reduceByKey(_ + _) // count by wordine.foreachRDD(rdd => {
      .map({case (w,c) => (w,new Date().getTime,c)}) // add the current saveToCassandra("Tweet,count")})
               NFL.print()
